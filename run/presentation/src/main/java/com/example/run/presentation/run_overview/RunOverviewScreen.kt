...
fun RunOverviewScreen(
    onActions: (RunOverviewActions) -> Unit,
) {
    ...
    RuniqueScaffold(
        topAppBar = {
            RuniqueToolbar(
                ...
                onMenuItemClick = { index ->
                    when (index) {
                        0 -> onActions(RunOverviewActions.OnAnalyticsClick)
                        1 -> onActions(RunOverviewActions.OnLogoutClick)
                    }
                },
                startContent = {
                    Icon(
        ...
        floatingActionButton = {
            RuniqueFloatingActionButton(
                icon = RunIcon,
                onClick = {
                    onActions(RunOverviewActions.OnStartClick)
                }
            )
        }
    ) { innerPadding->
